    ...
    gcloud: {
        enableLogging: function enableGcloudLogging(oeLogger) {
            ...
            if ([true, "TRUE", "true", "Y"].some(v => v === process.env.ENABLE_GCLOUD_LOGGING)) {
                oeLogger('LOGGER-CONFIG').getLogger().streams.push(new (require('@google-cloud/logging-bunyan').LoggingBunyan)().stream(process.env.ENABLE_GCLOUD_LOGGING_LVL || 'error'));
            }
            loggingChecked = true;
        }
    }
